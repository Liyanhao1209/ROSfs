import socket,threading,pickle,sys,time,subprocess
import zmq
import rosbag, genpy, genmsg

def kill_processes_on_port(port):
    try:
        command = f"lsof -i :{port} | awk 'NR>1 {{print $2}}' | xargs kill -9"
        
        result = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        
        if result.returncode == 0:
            print(f"Successfully killed processes on port {port}",flush=True)
        else:
            print(f"Error executing command: {result.stderr}",flush=True)
...
class Server:
    ...
    def _get_message_type(self,info):
        try:
            ...
            if (message_type._md5sum != info.md5sum):
                print('WARNING: For type [%s] stored md5sum [%s] does not match message definition [%s].\n  Try: "rosrun rosbag fix_msg_defs.py old_bag new_bag."' % (info.datatype, info.md5sum, message_type._md5sum), file=sys.stderr,flush=True)
        except genmsg.InvalidMsgSpec:
            message_type = genpy.dynamic.generate_dynamic(info.datatype, "")[info.datatype]
            print('WARNING: For type [%s] stored md5sum [%s] has invalid message definition."' % (info.datatype, info.md5sum), file=sys.stderr,flush=True)
    ...
    def _start_reading(self):
        ...
        print(f"[Reader Daemon]:read {read_cnt} msgs,Daemon Buffer size:{len(self._buffer_)}",flush=True)
    ...
    def response(self):
        while True:
            try:
                ...
                print(f"Error {e} happened while server responsing",flush=True)
            if cmd==b'kill':
                print(f"[ROS INFO]: receive command kill",flush=True)
                self.close_session()
                break
            pose_lb,pose_ub = pickle.loads(raw_pose_lb),pickle.loads(raw_pose_ub)
            print(f"[ROS INFO]:receive query {cmd},{pose_lb},{pose_ub}",flush=True)
...
if __name__ == "__main__":
    ...
    kill_processes_on_port(ip2port[local_ip])
    print(f"Server {local_ip} listening process is killed",flush=True)
    
    server = Server(ip2port[local_ip],f'./{local_ip}.bag',ip2topic[local_ip][0],ip2topic[local_ip][1])
